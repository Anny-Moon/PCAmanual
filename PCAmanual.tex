...
\item Toolkit for visualization \comp{.pca} files, written in \comp{Python}.
...
\subsection{PCA format \comp{.pca}}\label{sec:.pca}
We present special format for all files with 3-dimensional chains configurations. It is plane text file with extension \comp{.pca}. One can open it with any text editor.

The format of  \comp{.pca} file is the following:
...
We have an additional software (command-line program) which allows to download \footnote{from rcsb.org.} and convert files of \comp{.pdb} format to our \comp{.pca} format. The software and detailed instructions how to use it can be found here: \textbf{https://github.com/Anny-Moon/pdb2xyz}.

\subsection{FilesHandler}\label{sec:inputFilesHandler}
FileHandler is an independent module (stored in folder \comp{FileHandler}) which allows to write other input formats for \PCA code completely independent from the rest of the code. The new class should inherit from abstract class \comp{FileCoordinates}. Files classes connect with class \comp{Polymer} only through arrays of double numbers with coordinates \comp{(double*  x, double* y, double* z)}.
...
All input files should have \comp{.pca} format and be placed in \comp{/data} in the root of the program.  Both these settngs can be easily changed in application's \comp{main.cpp}.
...
That reader will go to \comp{Polymer} constructor as only one argument.
...
\begin{mySection}[Visualization with Python]\label{sec:python}
In the folder \comp{tools} you can find \comp{PlotterPy} - interactive 3D plotter for \comp{.pca} files.
The plotter is written in \comp{Python 2.7} with using \comp{Matplotlib}.\footnote{If you use MacOS, then you do not need to install anything}. 
...
To save this result in \comp{1abs.pdf} you can run this instead:
\begin{lstlisting}
...
\subsection{Movie}
To get a full picture of rescaling procedure it is better to create a movie, where each frame is a next step in the procedure. For this purpose we made \comp{MovieMaker.py} for you. As in previous plotter one can save the video or get it on the screen without saving. The program can save in GIF format (using \textit{imagemagick} library) or in MPEG4 format (using \textit{ffmpeg} library). You need to install the corresponding libary before you can save files.
...
The \comp{<increment>} allows you to skip frames, what can be useful if if there the original chain was very long and there are too many scaling steps. The default value is 1.

Try 
\begin{lstlisting}
@PlotterPy$@ python ./movieMaker.py 5dn7 3 gif movie
\end{lstlisting}
to record every third frame into a file \comp{movie.gif} or
\begin{lstlisting}
